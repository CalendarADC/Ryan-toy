...
export async function expandStep1PromptWithAi(args: ExpandArgs): Promise<ExpandResult> {
  ...
  const system = [
    ...
    "若命中强风格词：先在内部判断 1 个主风格 + 最多 2 个辅助风格；正文中风格名称最多出现一次，其余用可见细节体现。",
    ...
    "从用户信息中提炼 3-5 个最有辨识度的视觉锚点（如主元素、轮廓、结构、镶口、材质对比），其余信息只做从属。",
    ...
    "=== 反同质化规则（硬约束，必须全部满足）===",
    "1) 不改变品类与主体元素。",
    "2) 只选一个设计主命题并贯穿（说明最想被看见什么、为此取舍什么）。",
    "3) 明确外轮廓语法：以可佩戴结构为前提，写清对称性、视觉重心与延展方向；除非用户明确要求，禁止开口戒/绕指错位式断开结构。",
    `4) 明确镶嵌架构与石位节奏：镶口类型、主辅石关系；全件可见宝石≤${STEP1_EXPAND_MAX_GEM_COUNT}颗、色号≤${STEP1_EXPAND_MAX_GEM_COLOR_COUNT}种，禁止成片密排镶。`,
    "5) 在可佩戴、可量产框架内规避模板化：至少给出两处差异化细节（线条/层级/肌理/镶口节奏），不得靠夸张异形破坏佩戴平衡。",
    "",
    "=== 反同质化规则（强约束，每次命中 2-3 条）===",
    "6) 体块比例系统：主题体量与戒臂/吊环匹配，避免头重脚轻。",
    "7) 层级与空间结构：主次分明、前后遮挡合理，避免元素堆叠成拼贴。",
    ...
    "除非用户原文明确要求开口戒/绕线戒，默认输出闭合戒圈结构，不得写断开式悬空戒臂。",
    "结构件交代（按品类）：戒指写清戒圈/戒肩/戒头连接；吊坠写清吊环与主体连接；若用户明确是耳饰/手链，则分别交代耳针(钩/夹)/链节与扣头结构。",
    "主镜头默认正面略俯或 3/4 hero 特写；禁止把纯侧面、纯背面或托底结构当作唯一主镜头（除非用户明确要求）。",
    "",
    "=== 展示背景（硬性，唯一允许写法）===",
    `品类为戒指时展示背景句必须为：展示背景：根据设计，把戒指放到你认为合适的展示背景里`,
    `品类为吊坠时展示背景句必须为：展示背景：根据设计，把吊坠放到你认为合适的展示背景里`,
    "背景内部原则：低存在感、不抢主体。禁止描写具体台面/布景/环境（如丝绒、橡木、大理石、灰白无缝棚拍、深色木桌、柔光箱场景等）；禁止随机发明背景；把背景决策交给后续生图模型。",
    "扩写正文其它部分可写材质、工艺、造型与光影，但「展示背景」一行只能使用上述固定句。",
    "数值与比例：除命中细戒/中细戒硬性比例句外，不要机械套用固定毫米数、固定比例或固定间距模板；允许为视觉锚点提供必要的软数值（占比/区间/分区），仅在用户明确要求时写具体毫米数。",
    "",
    "=== 镶嵌材质（硬性）===",
    ...
    ...(args.kind === "ring" && getRingMotifShankScaleTier(args.prompt) === "ultra-thin"
      ? [
          ...
          `用户为细戒或女戒等：主题/戒臂比例要求**仅写一次**，融入正文前段（如「体量相对戒臂 1.2–1.6 倍、肩线融合、禁止中间大两侧小」）；禁止在「展示背景」句之后追加比例句；禁止结尾重复「设计主题相对戒臂…」标准句或括号补充；禁止改写为「1.4倍」等其它数值。`,
          "",
        ]
      : []),
    ...(args.kind === "ring" && getRingMotifShankScaleTier(args.prompt) === "medium-thin"
      ? [
          ...
          `用户为中细戒或中性戒指等：主题/戒臂比例要求**仅写一次**，融入正文前段；禁止在「展示背景」句之后追加比例句；禁止结尾重复标准句或括号补充。`,
          "",
        ]
      : []),
