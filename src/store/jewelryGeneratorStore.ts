...
function defaultCopyTemplate(): CopyTemplate {
  ...
  return {
    ...
    descriptionFormat:
      "英文 Description：Product Description → Material → Care Instructions → Order & Shipping → Customization；Material 须据识图+提示词确认主材质（S925 / 18K Rose Gold Plated / Solid brass 三选一），禁止瞎猜；emoji≤3；有宝石写锆石色与种类；项链写 40cm/60cm 链长。",
    tagsFormat:
...
export const useJewelryGeneratorStore = create<JewelryGeneratorStore>()(
  persist(
    (set, get) => ({
      ...
      setStep1ImageResolution: (v: "1K" | "2K" | "4K") => {
        set({ step1ImageResolution: v });
        const s = get();
        if (tasksHydrated) scheduleDebouncedTaskMetaSave(s.activeTaskId, pickTaskMeta(s));
      },
      setStep2ImageResolution: (v: "1K" | "2K" | "4K") => {
        set({ step2ImageResolution: v });
        const s = get();
        if (tasksHydrated) scheduleDebouncedTaskMetaSave(s.activeTaskId, pickTaskMeta(s));
      },
      setImageApiVendor: (v) => {
